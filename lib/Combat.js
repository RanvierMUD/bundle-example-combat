...
const { Damage, Logger, RandomUtil } = require('ranvier');
...
class Combat {
  ...
  static makeAttack(attacker, target) {
    ...
    if (attacker.hasAttribute('critical')) {
      ...
      critical = RandomUtil.probability(critChance);
      if (critical) {
        amount = Math.ceil(amount * 1.5);
      }
    }

    const damage = new Damage({ attribute: 'health', amount, attacker, critical });
